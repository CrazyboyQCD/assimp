...
use crate::{
    assets::formats::mmd::{
        ...
        parser::{
            ...
            pmd::{error::PmdParseError, structs::PmdModel},
            pmx::structs::{PMXIndex, PmxSetting},
        },
    },
...
pub struct PMDParser<'source> {
    binary_buffer_reader: BinaryBufferReader<'source, LE>,
}

impl<'source> PMDParser<'source> {
    ...
    pub fn parse(&mut self) -> Result<PmdModel, MMDImportError> {
        match PmdModel::read(self) {
            Ok(model) => Ok(model),
    ...
    fn read_string<const N: usize>(&mut self) -> Result<String, MMDParseError> {
        ...
        match str::from_utf8(bytes) {
            ...
            Err(_) => {
                ...
                Err(PmdParseError::UnknownEncodedString(buffer.to_vec()))?
            }
        }
    }
