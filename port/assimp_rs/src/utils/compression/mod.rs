...
use zlib_rs::inflate::{InflateConfig, InflateStream};
...
impl Compression {
    ...
    pub fn decompress(
        ...
        if flush_mode == InflateFlush::Finish {
            ...
            let mut total = 0;
            #[allow(invalid_value)]
            // SAFETY: zlib will overwrite the uninitialized memory
            let mut block: [u8; MYBLOCK] = unsafe { MaybeUninit::uninit().assume_init() };
