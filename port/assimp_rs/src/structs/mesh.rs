...
pub struct AiMesh {
    ...
    pub texture_coords: Box<[Box<[Vec3]>; AI_MAX_NUMBER_OF_TEXTURECOORDS]>,
    pub texture_coords_names: Option<Box<[Box<str>; AI_MAX_NUMBER_OF_TEXTURECOORDS]>>,
    pub num_of_uv_components: Box<[u32; AI_MAX_NUMBER_OF_TEXTURECOORDS]>,
    pub faces: Box<[AiFace]>,
    pub bones: Box<[AiBone]>,
    pub material_index: u32,
    pub anim_meshes: Box<[AnimMesh]>,
    ...
    pub fn set_texture_coords_name(&mut self, index: usize, name: &str) {
        if index < AI_MAX_NUMBER_OF_TEXTURECOORDS {
            ...
            if let Some(names) = &mut self.texture_coords_names {
                ...
                let mut names: Box<[Box<str>; AI_MAX_NUMBER_OF_TEXTURECOORDS]> = Box::default();
                names[index] = name;
