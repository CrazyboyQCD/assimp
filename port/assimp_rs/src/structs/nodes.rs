...
impl<T> Index<T> {
    ...
    pub fn get(self, vec: &Vec<T>) -> Option<&T> {
        if self.is_exist() && self.value() < vec.len() {
    ...
    pub unsafe fn get_unchecked(self, vec: &Vec<T>) -> &T {
        unsafe { vec.get_unchecked(self.value()) }
    }

    pub fn get_mut(self, vec: &mut Vec<T>) -> Option<&mut T> {
        if self.is_exist() && self.value() < vec.len() {
            // SAFETY: self is not guard index and self.value() is guaranteed to be less than vec.len()
    ...
    pub unsafe fn get_mut_unchecked(self, vec: &mut Vec<T>) -> &mut T {
