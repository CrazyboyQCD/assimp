...
pub mod encoding {
    ...
    fn convert_utf32_to_string(buf: &[u8], is_big_endian: bool) -> Result<String, EncodingError> {
        if buf.len() % mem::size_of::<u32>() != 0 {
            return Err(EncodingError::NotValidUtf32Be);
        }
    ...
    fn convert_utf16_to_string(buf: &[u8], is_big_endian: bool) -> Result<String, EncodingError> {
        ...
        let result = if is_big_endian {
            ...
            .map_err(|_| EncodingError::NotValidUtf16Be)
        } else {
            char::decode_utf16(
                buf.chunks_exact(2)
            ...
            .collect::<Result<String, _>>()
            .map_err(|_| EncodingError::NotValidUtf16Le)
        };
...
pub trait FormatValidator<const N: usize>: FormatHeader<N> {
    ...
    #[cfg(feature = "std")]
    fn can_read_from_reader<R: Read>(reader: &mut R) -> bool {
        let mut buffer = [0u8; N];
        reader.read_exact(&mut buffer).is_ok() && Self::check_header(&buffer)
    }

    /// Validate format from file
    #[cfg(feature = "std")]
    fn can_read_from_file(file_name: &str) -> bool {
        match File::open(file_name) {
            Ok(mut file) => Self::can_read_from_reader(&mut file),
            Err(_) => false,
        }
    }
}
    ...
    FormatValidator<N> + InternalImporter<E> + Importer<E>
{
    ...
    where
        E: From<ImportError>,
    {
        if Self::can_read_from_file(file_name) {
